...
public class ImportController {
    ...
    @Autowired
    private TaxTuishuiAllQfsdService taxTuishuiAllQfsdService;
    ...
    @GetMapping("/tuishui-all-qfsd-query")
    public String tuishuiAllQfsdQueryPage() {
        return "tuishui-all-qfsd-query";
    }
    ...
    @PostMapping("/api/tuishui-all-qfsd/calculate")
    @ResponseBody
    public Result<String> calculateTuishuiAllQfsd() {
        try {
            String taskId = taxTuishuiAllQfsdService.startCalculate();
            return Result.success("计算任务已启动", taskId);
        } catch (Exception e) {
            return Result.error("启动计算失败：" + e.getMessage());
        }
    ...
    @PostMapping("/api/tuishui-all-qfsd/query")
    @ResponseBody
    public Result<PageResult<TaxTuishuiAllQfsd>> queryTuishuiAllQfsdList(@RequestBody(required = false) TaxQueryRequest request) {
        try {
            return Result.success(taxTuishuiAllQfsdService.queryAllList(request));
        } catch (Exception e) {
            return Result.error("查询失败：" + e.getMessage());
        }
    }

    @PostMapping("/api/tuishui-all-qfsd/export")
    public void exportTuishuiAllQfsdList(@RequestParam(value = "userName", required = false) String userName,
                                          @RequestParam(value = "idCard", required = false) String idCard,
                                          HttpServletResponse response) {
        Workbook workbook = new XSSFWorkbook();
        try {
            ...
            request.setUserName(userName);
            request.setIdCard(idCard);
            List<TaxTuishuiAllQfsd> list = taxTuishuiAllQfsdService.queryAllListForExport(request);
            Sheet sheet = workbook.createSheet("退税全量名单（区分税地）");
            ...
            for (int i = 0; i < list.size(); i++) {
                TaxTuishuiAllQfsd item = list.get(i);
                Row row = sheet.createRow(i + 1);
                row.createCell(0).setCellValue(nullToEmpty(item.getUserName()));
                row.createCell(1).setCellValue(nullToEmpty(item.getIdCard()));
                row.createCell(2).setCellValue(nullToEmpty(item.getPhone()));
            ...
            response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
            response.setCharacterEncoding(StandardCharsets.UTF_8.name());
            String fileName = URLEncoder.encode("退税全量名单（区分税地）", StandardCharsets.UTF_8).replaceAll("\\+", "%20");
            response.setHeader("Content-Disposition", "attachment; filename*=UTF-8''" + fileName + ".xlsx");
    ...
    @GetMapping("/api/tuishui-all-qfsd/progress/{taskId}")
    @ResponseBody
    public Result<ImportProgress> getTuishuiAllQfsdProgress(@PathVariable String taskId) {
        ImportProgress progress = taxTuishuiAllQfsdService.getProgress(taskId);
        if (progress == null) {
            return Result.error("任务不存在");
        }
        return Result.success(progress);
    ...
    public Result<ImportProgress> getProgress(@PathVariable String taskId) {
        ...
        if (progress == null) {
            progress = taxTuishuiAllQfsdService.getProgress(taskId);
        }
        if (progress == null) {
            return Result.error("任务不存在");
        }
